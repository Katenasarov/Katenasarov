...
|**Предсказание оттока клиентской базы.** https://github.com/Katenasarov/predict_clients | Обучить модель, модель, которая будет предсказывать, разорвёт ли абонент договор с оператором связи. | *Python Pandas Matplotlib PhiK PyTorch SHAP Optuna Scikit-learn*|
